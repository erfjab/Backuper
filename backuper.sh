...
readonly SPONSORTEXT="Thanks for useing @erfjab backuper"
...
log() { echo -e "${COLORS[cyan]}[INFO]${COLORS[reset]} $*" | tee -a /tmp/backup.log; }
warn() { echo -e "${COLORS[orange]}[WARN]${COLORS[reset]} $*" >&2 | tee -a /tmp/backup.log; }
error() { echo -e "${COLORS[red]}[ERROR]${COLORS[reset]} $*" >&2 | tee -a /tmp/backup.log; exit 1; }
wrong() { echo -e "${COLORS[red]}[WRONG]${COLORS[reset]} $*" >&2 | tee -a /tmp/backup.log; }
success() { echo -e "${COLORS[spring]}${COLORS[green]}[SUCCESS]${COLORS[reset]} $*" | tee -a /tmp/backup.log; }
...
marzneshin_template() {
    ...
    if [[ "$DB_TYPE" != "sqlite" && -z "$DB_PASSWORD" ]]; then
        error "Database password not found in $docker_compose_file"
        return 1
    fi
...
marzhelp_template() {
    ...
    MARZHELP_BACKUP_COMMAND="mysqldump -h \"$db_host\" -P \"$db_port\" -u root -p\"$MYSQL_ROOT_PASSWORD\" marzhelp > \"$MARZHELP_DB_PATH\""
...
telegram_progress() {
    clear
    print "[TELEGRAM]\n"
    print "To use Telegram, you need to provide a bot token and chat IDs (comma-separated).\n"
    while true; do
        while true; do
        ...
        while true; do
            input "Enter the chat IDs (comma-separated, e.g., 123456789,123965874): " CHAT_IDS
            if [[ -z "$CHAT_IDS" ]]; then
                wrong "Chat IDs cannot be empty!"
            else
                ...
                for chat_id in "${CHAT_ID_ARRAY[@]}"; do
                    ...
                    if [[ ! "$chat_id" =~ ^-?[0-9]+$ ]]; then
                        wrong "Invalid chat ID format: $chat_id"
                        valid=false
                        break
                    fi
                done
        ...
        for chat_id in "${CHAT_ID_ARRAY[@]}"; do
            chat_id=$(echo "$chat_id" | tr -d ' ')
            response=$(curl -s -w "%{http_code}" -X POST "https://api.telegram.org/bot$BOT_TOKEN/sendMessage" -d chat_id="$chat_id" -d text="Hi, Backuper Test Message!" -o /tmp/telegram_response_$chat_id.json)
            log "Response for chat $chat_id: $response"
            if [[ "$response" -ne 200 ]]; then
                wrong "Invalid bot token or chat ID: $chat_id. Response: $(cat /tmp/telegram_response_$chat_id.json)"
                valid=false
                break
            fi
        done
        if $valid; then
            success "Bot token and chat IDs are valid."
            break
        fi
    done
    CHAT_IDS=$(echo "$CHAT_IDS" | tr -d ' ')
    PLATFORM_COMMAND="for chat_id in \$(echo \$CHAT_IDS | tr ',' ' '); do curl -s -F \"chat_id=\$chat_id\" -F \"document=@\$FILE\" -F \"caption=\$CAPTION\" -F \"parse_mode=HTML\" \"https://api.telegram.org/bot$BOT_TOKEN/sendDocument\" -o /tmp/telegram_send_response_\$chat_id.json; if [[ \$? -ne 0 ]]; then echo \"[ERROR] Failed to send \$FILE to chat \$chat_id\" >> /tmp/backup.log; exit 1; fi; done"
    CAPTION="
...
discord_progress() {
    ...
    while true; do
        ...
        log "Discord response code: $response"
...
generate_script() {
    ...
    if [[ -n "$MARZHELP_DB_PATH" ]]; then
        DB_CLEANUP="$DB_CLEANUP && rm -rf \"$MARZHELP_DB_PATH\" 2>/dev/null || true"
    fi
    cat <<EOL > "$BACKUP_PATH"
#!/bin/bash
set -e
...
log() { echo "[INFO] \$*" >> /tmp/backup.log; }
success() { echo "[SUCCESS] \$*" >> /tmp/backup.log; }
error() { echo "[ERROR] \$*" >> /tmp/backup.log; exit 1; }
...
if ! $COMPRESS "\$backup_name" ${BACKUP_DIRECTORIES[@]}; then
    error "Failed to compress ${REMARK} files. Please check the server."
fi

log "Checking backup file existence"
if ls \${base_name}* > /dev/null 2>&1; then
    for FILE in \${base_name}*; do
        log "Sending file: \$FILE"
        $PLATFORM_COMMAND
        for chat_id in \$(echo \$CHAT_IDS | tr ',' ' '); do
            if [[ -f "/tmp/telegram_send_response_\$chat_id.json" ]]; then
                if grep -q '"ok":true' "/tmp/telegram_send_response_\$chat_id.json"; then
                    success "Backup part sent successfully to chat \$chat_id: \$FILE"
                else
                    error "Failed to send ${REMARK} backup part to chat \$chat_id: \$FILE. Response: \$(cat /tmp/telegram_send_response_\$chat_id.json)"
                fi
            else
                error "No response file for chat \$chat_id. Send attempt failed."
            fi
        done
    done
    success "All backup parts sent successfully"
else
    error "Backup file not found: \$backup_name. Please check the server."
fi
...
EOL
    ...
    if bash "$BACKUP_PATH" 2>&1 | tee -a /tmp/backup.log; then
        ...
        error "Failed to run backup script. Check /tmp/backup.log for details."
    fi
}
